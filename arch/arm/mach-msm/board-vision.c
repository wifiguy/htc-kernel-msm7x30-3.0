...
static int vision_ts_atmel_power(int on)
{
	...
	if (on == 1) {
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(VISION_LED_3V3_EN), 1);
		msleep(5);
...
static struct cm3628_platform_data cm3628_pdata = {
	.intr = VISION_GPIO_PROXIMITY_INT_N,
	...
	.ps_thd_with_cal = 0x3,
	.ps_thd_no_cal = 0x7,
	.ps_conf2_val = 0,/*vision don't need test mode*/
...
static struct bma150_platform_data gsensor_platform_data = {
	.intr = VISION_GPIO_GSENSOR_INT_N,
	.chip_layout = 1,
};
...
static struct i2c_board_info i2c_devices[] = {
	...
	},
	{
		...
		.irq = MSM_GPIO_TO_INT(VISION_GPIO_GSENSOR_INT_N),
	},
	{
...
static int pm8058_gpios_init(void)
{
	...
	struct pm8xxx_gpio_init_info vision_tp_rstz = {
		PM8058_GPIO_PM_TO_SYS(VISION_TP_RSTz),
		{
	...
	struct pm8xxx_gpio_init_info vision_sdmc_cd_n = {
	...
	struct pm8xxx_gpio_init_info vision_vol_up = {
		PM8058_GPIO_PM_TO_SYS(VISION_VOL_UP),
		{
			.direction	= PM_GPIO_DIR_IN,
			.output_buffer	= 0,
			.output_value	= 0,
	...
	struct pm8xxx_gpio_init_info vision_vol_dn = {
		PM8058_GPIO_PM_TO_SYS(VISION_VOL_DN),
		{
			.direction	= PM_GPIO_DIR_IN,
			.output_buffer	= 0,
			.output_value	= 0,
	...
	struct pm8xxx_gpio_init_info vision_green_led = {
		PM8058_GPIO_PM_TO_SYS(VISION_GREEN_LED),
		{
			.direction	= PM_GPIO_DIR_OUT,
			.output_buffer	= PM_GPIO_OUT_BUF_CMOS,
			.output_value	= 1,
	...
	struct pm8xxx_gpio_init_info vision_amber_led = {
		PM8058_GPIO_PM_TO_SYS(VISION_AMBER_LED),
		{
			.direction	= PM_GPIO_DIR_OUT,
			.output_buffer	= PM_GPIO_OUT_BUF_CMOS,
			.output_value	= 1,
	...
	struct pm8xxx_gpio_init_info vision_aud_hp_detz = {
	...
	struct pm8xxx_gpio_init_info vision_aud_spk_en = {
		PM8058_GPIO_PM_TO_SYS(VISION_AUD_SPK_EN),
		{
			.direction	= PM_GPIO_DIR_OUT,
			.output_buffer	= PM_GPIO_OUT_BUF_CMOS,
			.output_value	= 1,
	...
	struct pm8xxx_gpio_init_info vision_ps_shdn = {
		PM8058_GPIO_PM_TO_SYS(VISION_PS_SHDN),
		{
			.direction	= PM_GPIO_DIR_OUT,
			.output_buffer	= PM_GPIO_OUT_BUF_CMOS,
			.output_value	= 0,
	...
	pm8xxx_gpio_config(vision_tp_rstz.gpio, &vision_tp_rstz.config);
	pm8xxx_gpio_config(vision_sdmc_cd_n.gpio, &vision_sdmc_cd_n.config);
	pm8xxx_gpio_config(vision_vol_up.gpio, &vision_vol_up.config);
	pm8xxx_gpio_config(vision_vol_dn.gpio, &vision_vol_dn.config);
	pm8xxx_gpio_config(vision_green_led.gpio, &vision_green_led.config);
	pm8xxx_gpio_config(vision_amber_led.gpio, &vision_amber_led.config);
	pm8xxx_gpio_config(vision_aud_hp_detz.gpio, &vision_aud_hp_detz.config);

	rc = pm8xxx_gpio_config(vision_aud_spk_en.gpio, &vision_aud_spk_en.config);
	if (rc) {
		pr_err("%s VISION_AUD_SPK_EN config failed\n", __func__);
		return rc;
	}

	rc = pm8xxx_gpio_config(vision_ps_shdn.gpio, &vision_ps_shdn.config);
	if (rc) {
		pr_err("%s VISION_PS_SHDN config failed\n", __func__);
		return rc;
	}
/*
	...
	if (rc) {
		pr_err("%s VISION_GPIO_SDMC_CD_N config failed\n", __func__);
		return rc;
	}
#endif
*/
/*	keypad_gpio.gpio = VISION_VOL_UP;
	pm8xxx_gpio_config(keypad_gpio.gpio, &keypad_gpio.config);
	keypad_gpio.gpio = VISION_VOL_DN;
	pm8xxx_gpio_config(keypad_gpio.gpio, &keypad_gpio.config);	*/
#if 0
	if (machine_is_msm7x30_fluid()) {
		/* Haptics gpio */
...
static struct pm8xxx_irq_platform_data pm8xxx_irq_pdata = {
	...
	.devirq			= MSM_GPIO_TO_INT(VISION_PMIC_GPIO_INT),
...
static struct msm_ssbi_platform_data msm7x30_ssbi_pm8058_pdata __devinitdata = {
	...
	.slave	= {
		.name			= "pm8058-core",
		.irq = MSM_GPIO_TO_INT(VISION_PMIC_GPIO_INT),
...
#ifdef CONFIG_S5K4E5YX
static int vision_sensor_version(void)
{
	if (sensor_version == 999) {
		if (gpio_get_value(VISION_GPIO_CAM_ID) == 0) {
			sensor_version = VISION_S5K4E5YX_EVT2;
		} else {
			sensor_version = VISION_S5K4E5YX_EVT1;
		}
	}
...
static int Primou_s5k4e5yx_vreg_on(void)
{
	...
	if (vision_sensor_version() == VISION_S5K4E5YX_EVT2) {
		/* EVT2 V_CAM_D1V5 */
...
static struct msm_camera_sensor_info msm_camera_sensor_s5k4e5yx_data = {
	...
	.sensor_pwd = VISION_GPIO_CAM1_PWD,
	.vcm_pwd = VISION_GPIO_CAM1_VCM_PWD,
	...
	.use_rawchip = 1,
	.sensor_version = vision_sensor_version,
...
static int vision_use_ext_1v2(void)
{

		return 1;

}
static int vision_rawchip_vreg_on(void)
{
	int rc;
	...
	if (system_rev >= 2) {
		/* V_RAW_1V2_EN*/
		rc = gpio_request(VISION_GPIO_RAW_1V2_EN, "V_RAW_1V2_EN");
		if (rc) {
			pr_err("[CAM] sensor_power_enable\
				(\"gpio %d\", 1.8V) FAILED %d\n",
				VISION_GPIO_RAW_1V2_EN, rc);
		} else {
			gpio_direction_output(VISION_GPIO_RAW_1V2_EN, 1);
			gpio_free(VISION_GPIO_RAW_1V2_EN);
		}		
	} else {
		/* V_RAWCSI_1V2 */
...
static int vision_rawchip_vreg_off(void)
{
	int rc = 1;
	...
	if (system_rev >= 2) {
		...
		if (rc) {
			...
			gpio_direction_output(VISION_GPIO_RAW_1V2_EN, 0);
			gpio_free(VISION_GPIO_RAW_1V2_EN);
		}		
	} else {
		/* V_RAW_1V2 */
...
static uint32_t rawchip_on_gpio_table[] = {
	GPIO_CFG(VISION_GPIO_RAW_RSTN, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), /* RAW CHIP Reset */
	GPIO_CFG(VISION_GPIO_RAW_INTR0, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), /* RAW CHIP INT0 */
	GPIO_CFG(VISION_GPIO_RAW_INTR1, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), /* RAW CHIP INT1 */
	GPIO_CFG(VISION_GPIO_MCAM_SPI_CS, 2 , GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
#if 0
	GPIO_CFG(VISION_GPIO_CAM1_VCM_PWD, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(VISION_GPIO_CAM1_PWD, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
#endif
	GPIO_CFG(VISION_GPIO_CAM_MCLK, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), /* MCLK */
	GPIO_CFG(VISION_GPIO_CAM_I2C_SCL, 2, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), /* I2C SCL*/
	GPIO_CFG(VISION_GPIO_CAM_I2C_SDA, 2, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), /* I2C SDA*/
	GPIO_CFG(VISION_GPIO_RAW_RSTN, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), /* RAW CHIP Reset */
	GPIO_CFG(VISION_GPIO_RAW_INTR0, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), /* RAW CHIP INT0 */
	GPIO_CFG(VISION_GPIO_RAW_INTR1, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), /* RAW CHIP INT1 */
};
...
static uint32_t rawchip_off_gpio_table[] = {
	GPIO_CFG(VISION_GPIO_CAM1_PWD, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(VISION_GPIO_CAM1_VCM_PWD, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(VISION_GPIO_CAM_MCLK, 1, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA), /* MCLK */
	GPIO_CFG(VISION_GPIO_CAM_I2C_SCL, 2, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_4MA), /* I2C SCL*/
	GPIO_CFG(VISION_GPIO_CAM_I2C_SDA, 2, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA), /* I2C SDA*/
	GPIO_CFG(VISION_GPIO_CAM_MCLK, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_16MA), /* MCLK */
	GPIO_CFG(VISION_GPIO_RAW_RSTN, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), /* RAW CHIP Reset */
	GPIO_CFG(VISION_GPIO_RAW_INTR0, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /* RAW CHIP INT0 */
	GPIO_CFG(VISION_GPIO_RAW_INTR1, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /* RAW CHIP INT1 */
	GPIO_CFG(VISION_GPIO_MCAM_SPI_CS, 0 , GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_4MA),
};
...
static struct msm_camera_rawchip_info msm_rawchip_board_info = {
	.rawchip_reset	= VISION_GPIO_RAW_RSTN,
	.rawchip_intr0	= VISION_GPIO_RAW_INTR0,
	.rawchip_intr1	= VISION_GPIO_RAW_INTR1,
	...
	.camera_rawchip_power_on = vision_rawchip_vreg_on,
	.camera_rawchip_power_off = vision_rawchip_vreg_off,
	.rawchip_gpio_on = config_rawchip_on_gpios,
	.rawchip_gpio_off = config_rawchip_off_gpios,
	.rawchip_use_ext_1v2 = vision_use_ext_1v2,
};
...
/*static struct tpa2051d3_platform_data tpa2051d3_platform_data = {
	.gpio_tpa2051_spk_en = VISION_AUD_SPK_SD,
};*/
...
{
	if (gpio_tlmm_config(GPIO_CFG(VISION_PMIC_GPIO_INT, 1, GPIO_CFG_INPUT,
				  GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE))
		pr_err("%s: gpio_tlmm_config (gpio=%d) failed\n",
		       __func__, VISION_PMIC_GPIO_INT);
...
static struct i2c_board_info i2c_bma250_devices[] = {
	{
		...
		.irq = MSM_GPIO_TO_INT(VISION_GPIO_GSENSOR_INT),
	},
};
...
static uint32_t msm_spi_on_gpio[] = {
       GPIO_CFG(VISION_GPIO_MCAM_SPI_DO, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
       GPIO_CFG(VISION_GPIO_MCAM_SPI_DI, 1, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_4MA),
       GPIO_CFG(VISION_GPIO_MCAM_SPI_CS, 2 /*3*/, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
       GPIO_CFG(VISION_GPIO_MCAM_SPI_CLK, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
};
#endif
...
static struct htc_headset_pmic_platform_data htc_headset_pmic_data = {
	...
	.hpin_irq		= MSM_GPIO_TO_INT(PM8058_GPIO_PM_TO_SYS(VISION_AUD_HP_DETz)),
...
static uint32_t usb_ID_PIN_input_table[] = {
	GPIO_CFG(VISION_GPIO_USB_ID_PIN, 0, GPIO_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
};

static uint32_t usb_ID_PIN_ouput_table[] = {
	GPIO_CFG(VISION_GPIO_USB_ID_PIN, 0, GPIO_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
};
...
#ifdef CONFIG_FLASHLIGHT_TPS61310
	i2c_register_board_info(0, vision_flashlight,
	ARRAY_SIZE(vision_flashlight));
...
#endif
/*	if (board_mfg_mode() == 1) {
		for (i = 0; i < ARRAY_SIZE(vision_ts_synaptics_data);  i++)
			vision_ts_synaptics_data[i].mfg_flag = 1;
	}
*/
...
/*	device_mid = get_model_id();
	if (strstr(device_mid, "PK7612")) {
		if (1 == board_mfg_mode()) {
			vision_synaptics_virtual_keys_attr.show = &vision_synaptics_virtual_keys_show_china_mfg;
			vision_himax_virtual_keys_attr.show = &vision_himax_virtual_keys_show_china_mfg;
		} else {
			vision_synaptics_virtual_keys_attr.show = &vision_synaptics_virtual_keys_show_china;
			vision_himax_virtual_keys_attr.show = &vision_himax_virtual_keys_show_china;
		}
	}
*/
	/*Virtual_key*/
