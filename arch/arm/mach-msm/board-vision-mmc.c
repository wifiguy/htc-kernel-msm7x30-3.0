...
#define VISION_SDMC_CD_N_SYS	PM8058_GPIO_PM_TO_SYS(VISION_SDMC_CD_N)
...
static unsigned int vision_sdslot_status(struct device *dev)
{
	...
	status = (unsigned int) gpio_get_value(VISION_SDMC_CD_N_SYS);
	return !status;
}
...
static struct mmc_platform_data vision_sdslot_data = {
	...
	.status_irq	= MSM_GPIO_TO_INT(VISION_SDMC_CD_N_SYS),
...
static int vision_wifi_cd = 0;	/* WIFI virtual 'card detect' status */
...
static int vision_wifi_status_register(void (*callback)(int card_present,
							  void *dev_id),
					 void *dev_id)
{
	if (wifi_status_cb)
...
int vision_wifi_power_state=0;
int vision_bt_power_state=0;
...
static DEFINE_SPINLOCK(vision_w_b_slock);
int vision_sleep_clk_state_wifi = CLK_OFF;
int vision_sleep_clk_state_bt = CLK_OFF;

int vision_wifi_bt_sleep_clk_ctl(int on, int id)
{
	int err = 0;
	unsigned long flags;
	...
	spin_lock_irqsave(&vision_w_b_slock, flags);
	if (on) {
		if ((CLK_OFF == vision_sleep_clk_state_wifi)
			&& (CLK_OFF == vision_sleep_clk_state_bt)) {
			printk(KERN_DEBUG "EN SLEEP CLK\n");
			pmic_gpio_sleep_clk_output.function = PM_GPIO_FUNC_2;
			err = pm8xxx_gpio_config(VISION_WIFI_SLOW_CLK,
					&pmic_gpio_sleep_clk_output);
			if (err) {
				spin_unlock_irqrestore(&vision_w_b_slock,
							flags);
		...
		if (id == ID_BT)
			vision_sleep_clk_state_bt = CLK_ON;
		else
			vision_sleep_clk_state_wifi = CLK_ON;
	} else {
		if (((id == ID_BT) && (CLK_OFF == vision_sleep_clk_state_wifi))
			|| ((id == ID_WIFI)
			&& (CLK_OFF == vision_sleep_clk_state_bt))) {
		...
		if (id)
			vision_sleep_clk_state_bt = CLK_OFF;
		else
			vision_sleep_clk_state_wifi = CLK_OFF;
	}
	spin_unlock_irqrestore(&vision_w_b_slock, flags);

	return 0;
}
EXPORT_SYMBOL(vision_wifi_bt_sleep_clk_ctl);
...
int vision_bt_fastclock_power(int on)
{
	int rc;

	printk(KERN_DEBUG "vision_bt_fastclock_power on = %d\n", on);
	if (vreg_wifi_osc) {
		if (on) {
			...
			printk(KERN_DEBUG "BT vreg_enable vreg_mmc, rc=%d\n",
			       rc);
			if (rc) {
				printk("Error turn vision_bt_fastclock_power rc=%d\n", rc);
				return rc;
			}
		} else {
			if (!vision_wifi_power_state) {
	...
	vision_bt_power_state = on;
	return 0;
}
EXPORT_SYMBOL(vision_bt_fastclock_power);

static int vision_wifi_reset_state;
void vision_wifi_reset(int on)
{
	printk(KERN_INFO "%s: do nothing\n", __func__);
}
#ifndef CONFIG_WIFI_CONTROL_FUNC
EXPORT_SYMBOL(vision_wifi_reset);
#endif
...
//	register_msm_irq_mask(INT_SDC2_1);
	...
	msm_add_sdcc(2, &vision_movinand_data);
...
//	register_msm_irq_mask(INT_SDC4_1);
	...
	irq_set_irq_wake(MSM_GPIO_TO_INT(VISION_SDMC_CD_N_SYS), 1);
...
static int visionmmc_dbg_wifi_reset_set(void *data, u64 val)
{
	vision_wifi_reset((int) val);
	return 0;
}

static int visionmmc_dbg_wifi_reset_get(void *data, u64 *val)
{
	*val = vision_wifi_reset_state;
	return 0;
}

static int visionmmc_dbg_wifi_cd_set(void *data, u64 val)
{
	vision_wifi_set_carddetect((int) val);
	return 0;
}

static int visionmmc_dbg_wifi_cd_get(void *data, u64 *val)
{
	*val = vision_wifi_cd;
	return 0;
}

static int visionmmc_dbg_wifi_pwr_set(void *data, u64 val)
{
	vision_wifi_power((int) val);
	return 0;
}

static int visionmmc_dbg_wifi_pwr_get(void *data, u64 *val)
{

	*val = vision_wifi_power_state;
	return 0;
}

static int visionmmc_dbg_sd_pwr_set(void *data, u64 val)
{
	vision_sdslot_switchvdd(NULL, (unsigned int) val);
	return 0;
}

static int visionmmc_dbg_sd_pwr_get(void *data, u64 *val)
{
	*val = sdslot_vdd;
	return 0;
}

static int visionmmc_dbg_sd_cd_set(void *data, u64 val)
{
	return -ENOSYS;
}

static int visionmmc_dbg_sd_cd_get(void *data, u64 *val)
{
	*val = vision_sdslot_status(NULL);
	return 0;
}

DEFINE_SIMPLE_ATTRIBUTE(visionmmc_dbg_wifi_reset_fops,
			visionmmc_dbg_wifi_reset_get,
			visionmmc_dbg_wifi_reset_set, "%llu\n");

DEFINE_SIMPLE_ATTRIBUTE(visionmmc_dbg_wifi_cd_fops,
			visionmmc_dbg_wifi_cd_get,
			visionmmc_dbg_wifi_cd_set, "%llu\n");

DEFINE_SIMPLE_ATTRIBUTE(visionmmc_dbg_wifi_pwr_fops,
			visionmmc_dbg_wifi_pwr_get,
			visionmmc_dbg_wifi_pwr_set, "%llu\n");

DEFINE_SIMPLE_ATTRIBUTE(visionmmc_dbg_sd_pwr_fops,
			visionmmc_dbg_sd_pwr_get,
			visionmmc_dbg_sd_pwr_set, "%llu\n");

DEFINE_SIMPLE_ATTRIBUTE(visionmmc_dbg_sd_cd_fops,
			visionmmc_dbg_sd_cd_get,
			visionmmc_dbg_sd_cd_set, "%llu\n");

static int __init visionmmc_dbg_init(void)
{
	struct dentry *dent;

	dent = debugfs_create_dir("visionmmc_dbg", 0);
	if (IS_ERR(dent))
		return PTR_ERR(dent);

	debugfs_create_file("wifi_reset", 0644, dent, NULL,
			    &visionmmc_dbg_wifi_reset_fops);
	debugfs_create_file("wifi_cd", 0644, dent, NULL,
			    &visionmmc_dbg_wifi_cd_fops);
	debugfs_create_file("wifi_pwr", 0644, dent, NULL,
			    &visionmmc_dbg_wifi_pwr_fops);

	debugfs_create_file("sd_pwr", 0644, dent, NULL,
			    &visionmmc_dbg_sd_pwr_fops);
	debugfs_create_file("sd_cd", 0644, dent, NULL,
			    &visionmmc_dbg_sd_cd_fops);

	return 0;
}

device_initcall(visionmmc_dbg_init);
